classdef lteUEPhysical < handle
    ...
    methods
        ...
        function [] = receive(obj, waveform, varargin)
            ...
            % if no data is expected, skip decoding process
            % this is for improving performance in multi UE mode
            ...
            if exist('dci', 'var');
                ...
                if ~isempty(tb)
                    if crcDLSCH == 0        % if decoding successful
                        % demultiplex MAC PDU into MAC SDU
                        sdu = calculator.macDemux(double(dlschBit{1}));
