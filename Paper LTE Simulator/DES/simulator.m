classdef simulator < handle
    ...
    properties
        enbPHY = ltePHYdownlink.empty;
        UE = model.lteUE.empty;
        filename = ['result/nonphy_',datestr(now,'ddmmyy_HH_MM_SS'),'.mat'];
        siminfo;
        ...
        function [] = run(obj)
            while obj.currentTime < obj.simTime
                ...
                if obj.packetArrivalTime > obj.ttiTime
                    ...
                    [tb, ue] = obj.schedule;
                    if ~isempty(tb)
                        obj.siminfo.tbStat(obj.ttiTime + 1,:) =  [obj.ttiTime tb.createdTTI length(tb.data) tb.retransmissionNo tb.dci.ModCoding tb.dci.HARQNo obj.UE.cqi];
                    else
                        obj.siminfo.tbStat(obj.ttiTime + 1,:) =  [obj.ttiTime -1 -1 -1 -1 -1 -1];
                    end
                    obj.siminfo.buffStat(obj.ttiTime + 1,:) =  [obj.ttiTime length(obj.buffer)];
                        
                    % insert scheduled ue monitoring and transport block
                    obj.enbPHY.insertData(ue,tb);
                                                         
                    % ENB receiver
                    obj.packetStat = obj.enbPHY.performTransmission(obj.packetStat);
                    if mod(obj.ttiTime,500) == 0
                        obj.siminfo.packetStat = obj.packetStat;
                        simInfo = obj.siminfo; %#ok<NASGU>
                        save(obj.filename, 'simInfo','-mat');
                    end
                    obj.currentTime = obj.ttiTime;
                    obj.ttiTime = obj.ttiTime + 1;
                else
                    
                    % create MAC SDU packet and add to the ENB buffer
