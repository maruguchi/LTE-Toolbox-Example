...
enb.NDLRB = 6;                 % Number of downlink physical resouce block in the cell
...
progress = 0;
done = (SNRmax-SNRmin+1)*(iterationNumber+1)*29;

prevtrans = load('1st_transb.mat','dataerr'); 
...
for mcs = 0:0
    ...
    dataerr(mcs + 1).data = {};
    dataerr(mcs + 1).CBSstate = {};
    
    prevtransdatalen = length(prevtrans.dataerr(mcs+1).data);
    
    if prevtransdatalen == 0
       crcBER(mcs + 1,isnr + SNRpad) = 0;
       BLER(mcs + 1,isnr + SNRpad) = 0;
       save('2st_trans.mat','crcBER','BLER','dataerr','SNRpad')
       continue
    end
...
%                     
                    ...
                    if i < prevtransdatalen
                        tchBits = prevtrans.dataerr(mcs+1).data{i};
                    else
                        tchBits = prevtrans.dataerr(mcs+1).data{mod(i,prevtransdatalen) + 1};
                    end
                    ...
                    pdschBits = lteRateMatchTurbo(turboBits,Info.G,rv);
            ...
            for NSubframe = 0:9
                ...
                if ~(mod(NSubframe,5) == 0)
                    ...
                    if i < prevtransdatalen
                        CBSstate = prevtrans.dataerr(mcs+1).CBSstate{i};
                    else
                        CBSstate = prevtrans.dataerr(mcs+1).CBSstate{mod(i,prevtransdatalen) + 1};
                    end
                    
                    
                    turboRxBits = lteRateRecoverTurbo(pdschRxSoftBits, tbs, rv);
                    turboRxBitsHard = lteRateRecoverTurbo(pdschRxHardBits, tbs, rv);
                    
                    % Matlab LTE Toolbox to perform turbo decoding
                    crcRxBits = lteTurboDecode(turboRxBits);
                    
                    % Matlab LTE Toolbox to remove CRC bits
                    [tchRxBits, ~] = lteCRCDecode(crcRxBits{1}, '24A');
                    
                    [trblkout,err,stateout]  = lteDLSCHDecode(enb,pdsch,tbs,pdschRxSoftBits,CBSstate);
                    ...
                    if ~(err == 0)
                        ...
                        dataerr(mcs + 1).data{length(dataerr(mcs + 1).data) + 1} = tchBits;
                        dataerr(mcs + 1).CBSstate{length(dataerr(mcs + 1).CBSstate) + 1} = stateout;
                    end
                end
            end
        ...
        modulationBER(mcs + 1,isnr + SNRpad) = 1 - sum((pdschBitsFull == pdschRxHardBitsFull)) / size(pdschBitsFull, 1); %#ok<*SAGROW>
        rateMatchingBER(mcs + 1,isnr + SNRpad) = 1 - sum((turboBitsFull == turboRxBitsFull)) / size(turboBitsFull, 1);
        turboBER(mcs + 1,isnr + SNRpad) = 1 - sum((crcBitsFull == crcRxBitsFull)) / size(crcBitsFull, 1);
        crcBER(mcs + 1,isnr + SNRpad) = 1 - sum((tchBitsFull == tchRxBitsFull)) / size(tchBitsFull, 1);
        BLER(mcs + 1,isnr + SNRpad) = double(blockError) / (double(i) * 8);
        %cqiValue{mcs + 1,isnr + SNRpad} = cqiFull;
        %sinrsValue{mcs + 1isnr + SNRpad} = sinrFull;
    end
    save('2st_transb.mat','crcBER','BLER','dataerr','SNRpad')
end
