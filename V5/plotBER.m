...
% BER(4+size(dataBER,1):3+size(dataBER,1)+size(cqi,1),:) = cqi;
...
for iPlot = 1:size(BER,1)                                           % plot all MCS
    ...
    switch iPlot
        ...
        otherwise
            if iPlot < 4+size(dataBER,1)
                legendEntry{iPlot} = ['MCS - ',num2str(mcsTest(iPlot-3)),' BER (',modulation{iPlot-3},' code rate ',num2str(codeRate(iPlot-3)),' )'];
            else
                legendEntry{iPlot} = ['MCS - ',num2str(mcsTest(iPlot-3)),' CQI(',modulation{iPlot-3},' code rate ',num2str(codeRate(iPlot-3)),' )'];
            end
    end
end
...
figure('Name','CQI vs SNR','NumberTitle','off','Position',...
    [(screen(3)-plotSize(1))/2,60,plotSize(1),plotSize(2)-80]);
plot(snrMin:snrMax,cqi)
xlabel('SNR (dB)') % x-axis label
ylabel('CQI') % y-axis label
grid on;