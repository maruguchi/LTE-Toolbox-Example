function [ waveform, waveformInfo ] = lteULPHYTX(user)
%LTEULPHYTX Summary of this function goes here
...
user.ue.NSubframe = user.enb.NSubframe ;        % Uplink Subframe number ACK send after eNodeb Subframe received

% PUCCH Format 1a to carry SISO ACK 

user.pucch.ResourceIdx = user.ue.PUCCHResourceIndex;    
...
pucch1Sym = ltePUCCH1(user.ue, user.pucch,user.dataCRC);
pucch1DRSSym = ltePUCCH1DRS(user.ue, user.pucch);

% Generate indices for PUCCH 1 and its DRS
pucch1Indices = ltePUCCH1Indices(user.ue, user.pucch);
pucch1DRSIndices = ltePUCCH1DRSIndices(user.ue, user.pucch);
...
% SC-FDMA modulation
[waveform, waveformInfo] = lteSCFDMAModulate(user.ue, grid);
